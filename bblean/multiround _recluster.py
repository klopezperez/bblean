...
class _TreeMergingRound:
    ...
    def __call__(self, batch_info: tuple[str, tp.Sequence[tuple[Path, Path]]]) -> None:
        ...
        # Save the bfs and mol_ids for the next round
        fps_bfs, mols_bfs = tree._bf_to_np()
...
class _FinalTreeMergingRound():
    ...
    def __call__(self, batch_info: tuple[str, tp.Sequence[tuple[Path, Path]]]) -> None:
        ...
        # Release memory before getting the buffers and mol idxs
        tree.delete_internal_nodes()
...
def run_multiround_reclustering(
    ...
    reclustering_iterations_initial: int = 3,
    ...
    initial_fn = _InitialRound(
        ...
        threshold=threshold,
        out_dir=out_dir,
        reclustering_iterations=reclustering_iterations_initial,
